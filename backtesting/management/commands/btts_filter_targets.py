...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--no-score-missing", action="store_true", help="Do not invoke ScoreEngineV8 for missing BTTS predictions.")
    ...
    def handle(self, *args, **options):
        ...
        rows = []
        matched = missing = excluded = no_pred = score_errors = 0
        for home, away in targets:
            ...
            if prediction is None and not options["no_score_missing"]:
                try:
                    engine.evaluate_and_persist(fixture)
                except Exception as exc:
                    score_errors += 1
                    self.stdout.write(f"SCORE_ERROR | {fixture.home_team.name} vs {fixture.away_team.name} | {exc.__class__.__name__}: {exc}")
                prediction = Prediction.objects.filter(fixture=fixture, market__iexact="BTTS").order_by("-created_at", "-id").first()
            ...
            # Rank within eligibility tier. The tier bonus only orders approved A/B above rejected X.
        ...
        self.stdout.write(
            f"\nSUMMARY | targets={len(targets)} matched={matched} missing={missing} excluded={excluded} "
