...
class DailyPipeline:
    ...
    def _has_predictions_for_date(self, target_date: date) -> bool:
    ...
    def _enrich(self, target_date: date) -> StageResult:
        ...
        return StageResult(
            len(fixture_ids),
            f"{len(fixture_ids)} future shortlisted fixtures enriched ({mode})",
            {"candidates": len(fixture_ids), "pool_mode": mode, "fixture_ids": fixture_ids},
        )

    def _rescore_enriched(self, target_date: date) -> StageResult:
        limit = self.INTERACTIVE_POOL_LIMIT if self._interactive_fast_enabled() else 30
        fixture_ids = self._future_candidate_fixture_ids(target_date, limit=limit)
        future_count = Fixture.objects.filter(id__in=fixture_ids, kickoff__gt=timezone.now()).count()
        if future_count:
            # Sprint 7.12 performance fix: one shared feature preload for the whole
            # Premium pool instead of invoking score_v8 once per fixture.
            self._run_command(
                "rescore_candidates_batch",
                target_date=target_date.isoformat(),
                limit=limit,
            )
        start, end = self._date_bounds(target_date)
        future_start = max(start, timezone.now())
        premium = Prediction.objects.filter(
            model_version=V8_MODEL_VERSION,
            fixture__kickoff__gte=future_start,
            fixture__kickoff__lt=end,
            tier="TIER_A",
        ...
        return StageResult(
            future_count,
            f"{future_count} enriched future fixtures batch-rescored; {premium} raw tier-A",
            {"rescored": future_count, "raw_tier_a": premium, "fixture_ids": fixture_ids, "batch": True},
        )

    def _deep_analysis(self, target_date: date) -> StageResult:
        pool = high_recall_candidate_pool(
            target_date,
            rule=CandidatePoolRule(limit=self.DEEP_ANALYSIS_LIMIT, require_premium_value_odds=True),
        )
        ...
        if pending_fixture_ids:
            raise RuntimeError(
                f"Deep Analysis incomplete for {len(pending_fixture_ids)} Premium Value fixture(s): {pending_fixture_ids[:10]}"
            )
        ...
        return StageResult(
            count,
            f"{count} predictions deep-analyzed; {preferred} preferred fixture markets; pending=0",
            {
                "predictions": count,
                "preferred_markets": preferred,
                "version": DEEP_ANALYSIS_VERSION,
                "required_fixtures": required_count,
                "pending_fixtures": 0,
            },
        )
    ...
    def _stages_for(self, target_date, mode):
        ...
        # Interactive dashboard requests should never rescore the complete card a
        # second time when today's predictions already exist. Reuse the discovery
        # layer, refresh only the 40-fixture candidate pool, then Deep + select.
        if self._interactive_fast_enabled() and self._has_predictions_for_date(target_date):
            return [
                ("INGEST", lambda: self._ingest(target_date, fixtures_only=True), True),
                ("ENRICH_CANDIDATES", lambda: self._enrich(target_date), False),
                ("RESCORE_V8", lambda: self._rescore_enriched(target_date), True),
    ...
    def run(self, target_date, *, mode="full", generation_job_id=None):
        ...
        for index, (name, fn, required) in enumerate(stages, start=1):
            self._sync_generation_job(
                generation_job,
                current_stage=name,
                progress_pct=max(2, int(((index - 1) / max(1, len(stages))) * 95)),
            ...
            if (name in dependent_on_ingest and ingest_failed) or (name == "SELECT_PREMIUM" and required_failed):
                PipelineStageRun.objects.create(
                    ...
                    message="skipped because required dependency failed",
                    details={"dependency": "INGEST_OR_DEEP"},
                )
                warning_count += 1
            else:
            ...
            self._sync_generation_job(
                generation_job,
                progress_pct=min(97, int((index / max(1, len(stages))) * 97)),
        ...
        pipeline.status = (
            ...
            if required_failed
            else PipelineRun.STATUS_PARTIAL
            if warning_count
            else PipelineRun.STATUS_SUCCESS
        )
        pipeline.save()
        ...
        if generation_job is not None:
            ...
            generation_job.save(
                update_fields=["status", "current_stage", "progress_pct", "message", "finished_at", "metadata"]
            )
        return pipeline
