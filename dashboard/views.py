...
def _premium_rows(target_date, *, limit: int = 3):
    return list(
...
def _selected_date_picks(target_date, *, limit: int = 3):
    """Return stable official Premium rows, but never display a critical contradiction.

    Sprint 7.12.5 closes the stale-ledger gap: the dashboard performs a narrow
    safety check against the SAME current model/calibration veto used by the
    replacement service. If an already-published row has become critically
    inconsistent, one deterministic reconcile is executed immediately and the
    dashboard reloads the official rows. Normal odds/score movement does not
    trigger this path, so publication stability remains intact.
    """
    rows = _premium_rows(target_date, limit=limit)
    if not rows:
        return rows

    replacement = PremiumReplacementService(max_picks=limit)
    has_critical_stale_row = any(
        replacement._critical_consistency_risk(row.prediction)[0]
        for row in rows
    )
    if not has_critical_stale_row:
        return rows

    replacement.reconcile(target_date, trigger="dashboard_critical_self_heal")
    return _premium_rows(target_date, limit=limit)
...
def developer_dashboard(request):
    ...
    start = timezone.make_aware(datetime.combine(target_date, dt_time.min)); end = start + timedelta(days=1)
    selected_prediction_ids = set(DailyPremiumSelection.objects.filter(target_date=target_date, model_version=V8_MODEL_VERSION).values_list("prediction_id", flat=True))
    selected_count = len(selected_prediction_ids)
    alternates, rejected = [], []
    for source_row in context.get("near_premium", []):
        prediction = source_row["prediction"]
        if not (start <= prediction.fixture.kickoff < end) or prediction.id in selected_prediction_ids:
            continue
        row = dict(source_row); reasons = DailyPremiumSelector.rejection_reasons(prediction, score_floor=76.0); risk = PremiumRiskGuard.evaluate(prediction)
        if not reasons and not risk.blocked:
            if selected_count >= 3:
                row["reason"] = "Cumple todos los filtros Premium; suplente por ranking global después de completar el Top 3"; alternates.append(row)
            else:
                row["reason"] = "Elegible Premium con plaza disponible; requiere reconciliación del selector"; rejected.append(row)
        else:
            row["reason"] = _human_current_rejection_reason(prediction); rejected.append(row)
    ...
    context["premium_alternates"] = alternates; context["near_premium"] = rejected
...
def premium_generation_status(request):
    ...
    premium_count = len(_selected_date_picks(job.target_date))
