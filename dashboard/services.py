...
class DashboardService:
    ...
    def deep_premium_audit(self, picks: list[DailyPremiumSelection]) -> list[dict[str, Any]]:
        ...
        for selection in picks:
            ...
            rows.append({
                ...
                "market_support_index": deep.get("market_support_index", evidence.get("market_support_index")),
                "total_penalty": deep.get("total_penalty", evidence.get("total_deep_penalty")),
                "warnings": warnings,
                "failures": failures,
            })
        return rows
    ...
    def operational_summary(self, premium_picks: list[DailyPremiumSelection]) -> dict[str, Any]:
        future = Prediction.objects.filter(model_version=self.model_version, fixture__kickoff__gte=timezone.now())
    ...
    def recent_results(self, *, limit: int = 12) -> list[PredictionOutcome]:
        return list(
            ...
            .select_related("prediction__fixture", "prediction__fixture__home_team", "prediction__fixture__away_team")
