...
# Single-market specialization. BTTS needs two independent scoring events, so
# the probability floor is intentionally higher than the old generic B floor.
TIER_RULES = (
...
# Model-vs-market disagreement remains valuable in the specialized system.
...
# Venue contradiction is now exclusively BTTS-oriented.
...
class DailyPremiumSelector:
    ...
    def _venue_contradiction_metrics(cls, prediction: Prediction) -> dict:
        if prediction.market != PREMIUM_MARKET:
            return {
                "blocked": True,
                "severity": 1.0,
                "reliability_penalty": VENUE_MAX_RELIABILITY_PENALTY,
                "rank_penalty": VENUE_MAX_RANK_PENALTY,
                "weak_side": None,
        ...
        if home_n < VENUE_RECENT_MIN_SAMPLE or away_n < VENUE_RECENT_MIN_SAMPLE:
            return {
                ...
                "rank_penalty": VENUE_MAX_RANK_PENALTY,
                "weak_side": None,
                "reason": "btts_venue_sample_incomplete",
            }
    ...
    def _market_eligible_deep_preference(cls, prediction: Prediction) -> bool:
        # In the old dual-market engine an O2.5 sibling could steal the fixture's
        # preferred-market flag. In BTTS-only mode that preference is irrelevant:
        # BTTS is judged on its own deep-analysis evidence.
    ...
    def rejection_reasons(cls, prediction: Prediction, *, score_floor: float = 76.0) -> list[str]:
        ...
        effective_reliability = disagreement["effective_reliability"] - venue["reliability_penalty"]
        if effective_reliability < PREMIUM_MIN_RELIABILITY:
            out.append(f"effective_reliability:{effective_reliability:.3f}")
        if venue["blocked"]:
            out.append(f"venue_btts_contradiction:{venue.get('reason') or venue.get('weak_side')}")
        if calibration.raw_probability < BTTS_BASE_PROBABILITY_FLOOR:
            out.append(f"raw_probability:{calibration.raw_probability:.3f}")
        if calibration.calibrated_probability < BTTS_MIN_CALIBRATED_PROBABILITY:
            out.append(f"calibrated_probability:{calibration.calibrated_probability:.3f}")
        if calibration.calibrated_edge < 0.05:
    ...
    def _tier_for(cls, prediction: Prediction, *, score_floor: float = 84.0) -> str | None:
        ...
        for rule in TIER_RULES:
            ...
            if (
                ...
                and calibration.calibrated_edge >= rule.min_edge
                and calibration.reliable_ev >= rule.min_ev
                and calibration.raw_probability >= rule.min_probability
                and calibration.calibrated_probability >= BTTS_MIN_CALIBRATED_PROBABILITY
    ...
    def _rank_score(cls, prediction: Prediction) -> tuple[float, dict]:
        ...
        # Specialized BTTS ranking gives slightly more weight to calibrated
        # probability/reliability and less to raw score. Over-related evidence is
        # stored for audit only, never as an operational market signal.
        base_composite = (
            0.18 * score_component
            + 0.25 * ev_component
            + 0.18 * edge_component
            + 0.14 * probability_component
            + 0.10 * calibrated_probability_component
        ...
        rationale = {
            ...
            "raw_probability_component": round(probability_component, 2),
            "calibrated_probability_component": round(calibrated_probability_component, 2),
            ...
            "over_context_only": {
                "home_over25_rate": evidence.get("home_over25_rate"),
                "away_over25_rate": evidence.get("away_over25_rate"),
                "home_avg_total_goals": evidence.get("home_avg_total_goals"),
                "away_avg_total_goals": evidence.get("away_avg_total_goals"),
            },
            "odds_policy": "Premium Value 1.60-2.40",
            "value_gate": "BTTS-only: Deep + two-sided scoring + venue contradiction + calibrated edge/EV + reliability",
            "formula": "0.18*score + 0.25*reliable_ev + 0.18*edge + 0.14*raw_p + 0.10*cal_p + 0.15*reliability - penalties",
        }
    ...
    def select(self, target_date: date) -> list[DailyPremiumSelection]:
        ...
        for score_floor in DYNAMIC_SCORE_FLOORS:
            ...
            if self._unique_fixture_count(ranked) >= self.max_picks:
                break

        chosen = []
        seen_fixtures = set()
        for item in ranked:
            if item[0].fixture_id in seen_fixtures:
                continue
            chosen.append(item)
            seen_fixtures.add(item[0].fixture_id)
