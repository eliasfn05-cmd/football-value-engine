...
def _role_profile(team, fixture, role: str) -> dict | None:
    ...
    # Robust attack estimate: remove one maximum observation.  This prevents a
    # 4-5/5-4 style match from single-handedly creating an elite attack profile.
    robust_gf = list(gf)
    if len(robust_gf) >= 3:
    ...
    # Bayesian-style shrinkage of scoring frequency toward a conservative
    # prior.  A 1/1 or 2/2 start can no longer behave like true 100% evidence.
    ...
    # Approximate how many BTTS rank points are being supplied by the outlier.
...
def anti_zero_metrics(prediction) -> dict:
    ...
    # Independent-score core: BTTS is constrained by the weaker scoring side.
    ...
    # Safety-first ordering for Premium #1.
    safety_score = 100.0 * (
        0.40 * p_btts
        + 0.30 * anti_zero_component
...
def anti_zero_decision(prediction):
    ...
    if not metrics.get("available"):
        # Optional evidence must not crash production.  Existing core gates
        # remain authoritative when history cannot be loaded.
        return None
...
def premium_one_safe(prediction) -> bool:
    metrics = anti_zero_metrics(prediction)
    return bool(metrics.get("available") and metrics["max_zero_risk"] <= PREMIUM_ONE_MAX_ZERO_RISK)
